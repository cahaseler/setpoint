...
export class FakeClient implements AccountClientLike {
	...
	/** Test helper: simulates the already-connected account for `id` reconnecting in place (same instance) — fires onAccountReconnected, not onAccountConnected. */
	simulateReconnect(id: string): void {
		const account = this.connected.get(id);
		if (!account) throw new Error(`FakeClient.simulateReconnect: no connected account for "${id}"`);
		for (const listener of this.reconnectedListeners) listener(account);
	}
