...
- **Crafting progress is push-only, no subscribe step** — unlike market/observation, the game server sends `crafting_update` automatically whenever an account has jobs in progress. `GET /accounts/:playerId/crafting/events` streams it as Server-Sent Events: the buffered backlog (`CraftingEventsStore`, last ~50 per account, in-memory only) immediately on connect, then each new push live. It's one of the GET routes in `isUnboundedRequest` (`src/server/index.ts`, alongside `/combat/events`, `/pirate-radio/events` and `/observation/events`) — a long-lived stream, not a tick-bound mutation, but needs the same idle-timeout override. `pirate_radio` works the same way: the server pushes intercepted pirate transmissions to any account in range, buffered per account through the same generic `EventBuffer` and streamed by `GET /accounts/:playerId/pirate-radio/events`.
- **The observation stream is the one push feed that does need a subscription** — the server only sends `observation_update` while a watch is active, and silently drops the watch the moment the ship leaves the POI. `GET /accounts/:playerId/observation/events` therefore subscribes on connect and `ObservationSubscriptionKeeper` (`src/state/observation-subscription.ts`) re-establishes the watch on every location change for as long as a subscriber is attached, so a caller still has no subscribe-first step to remember. A failed subscribe comes back as 409 rather than as a stream that can never emit. It never unsubscribes: nothing distinguishes a watch it opened from one an operator opened by hand through the raw passthrough. Pass `?activeScan=true` for a sweep that resolves cloaked contacts at the cost of being detectable.
- **The observation stream and `GET .../observation` are different shapes on purpose.** The stream relays the server's `observation_update` frame verbatim — `*_changed`/`*_departed` pairs — while the snapshot serializes the lib's merged `ObservationView`. Both carry all five presence classes the watch tracks (players, pirates, empire NPCs, wildlife, intact prizes) since `@spacemolt/lib` 14.2.0; before that the cache dropped everything but players. Use the stream when an arrival or departure is the thing being acted on, since a merged view cannot distinguish "left" from "was never here". Neither carries **arena NPCs** — the watch covers five of `get_nearby`'s six classes. That needs no workaround: an arena match should be followed through `GET /accounts/:playerId/battle-log/events`, which carries absolute hull, shield, zone, stance and target for every participant, well beyond what presence would tell you.
...
| `GET` | `/accounts/:playerId/observation/events` | Live observation change feed (Server-Sent Events) — subscribes and maintains the watch for you | — (use `@setpoint/client`'s `account.observation.events()`) |
