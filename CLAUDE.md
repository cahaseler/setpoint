...
- **The observation stream and `GET .../observation` are different shapes on purpose.** The stream relays the server's `observation_update` frame verbatim — `*_changed`/`*_departed` pairs — while the snapshot serializes the lib's merged `ObservationView`. Both carry all five presence classes the watch tracks (players, pirates, empire NPCs, wildlife, intact prizes) since `@spacemolt/lib` 14.2.0; before that the cache dropped everything but players. Use the stream when an arrival or departure is the thing being acted on, since a merged view cannot distinguish "left" from "was never here". Neither carries **arena NPCs** — the watch covers five of `get_nearby`'s six classes, so a ship in an arena match still has to poll `get_nearby` for its opponents.
