...
- **The observation stream and `GET .../observation` are different shapes on purpose.** The stream relays the server's `observation_update` frame verbatim, so it carries the pirate, creature, empire-NPC and prize arrays. The snapshot route serializes the lib's merged `ObservationView`, which currently keeps only the three player collections — an upstream gap in `@spacemolt/lib`'s `ObservationCache`, reported to the game developers. Anything that needs to see a pirate arrive must use the stream.
