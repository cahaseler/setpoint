...
import type { CompoundGoalResult, GoalContext, StepResult } from "../goals.js";
import type { Goal, GoalResult } from "../goals.js";
 ...
 *
 * Uses the deposit API's `source: "storage"` parameter for direct
...
export class TransferStorageToFaction implements Goal {
	...
	async execute(ctx: GoalContext): Promise<CompoundGoalResult> {
		...
		const storageResponse = await ctx.endpoints.viewStorage();
		const items = storageResponse.structuredContent.items ?? [];
		const credits = storageResponse.structuredContent.credits ?? 0;
	...
	private async transferItem(
		...
		try {
			...
			await ctx.endpoints.depositToFactionStorage(itemId, quantity, "storage");
			...
			try {
				await ctx.endpoints.depositToFactionStorage(itemId, transferAmount, "storage");
				return succeeded(
