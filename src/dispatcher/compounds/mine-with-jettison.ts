...
import { JettisonCargo } from "../primitives/index.js";
 ...
 * Requires ctx.refreshState to track cargo changes.
 */
export class MineWithJettison implements Goal {
	...
	async execute(ctx: GoalContext): Promise<GoalResult> {
		...
		if (!getState) {
			return failed("MineWithJettison requires readLocalState or refreshState", 0);
		}
		...
		let currentState: StoredGameState = ctx.state;
		...
		while (jettisonRounds <= maxJettisonRounds) {
			...
			// Read updated cargo from local store (mutation response already synced it)
			...
			for (const junkItem of junkInCargo) {
				const jettison = new JettisonCargo({
					itemId: junkItem.itemId,
					quantity: junkItem.quantity,
				});

				const jettisonResult = await jettison.execute({
			...
			// Read updated cargo from local store (jettison response already synced it)
			currentState = await getState();
			jettisonRounds++;
	...
	private isFull(state: StoredGameState, threshold: number): boolean {
	...
	private findJunkInCargo(state: StoredGameState): Array<{ itemId: string; quantity: number }> {
