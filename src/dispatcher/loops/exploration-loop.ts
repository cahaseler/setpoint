...
class ExplorationIteration implements Goal {
	...
	async execute(ctx: GoalContext): Promise<GoalResult> {
		...
		const intelResult = await ctx.endpoints.queryIntel();
		const minTick = this.options.minSubmittedAtTick;
		const recordedSystemIds = new Set(
			(
				intelResult.structuredContent.entries as Array<{
					system_id: string;
					submitted_at_tick?: number;
				}>
			)
				.filter((e) => minTick === undefined || (e.submitted_at_tick ?? 0) >= minTick)
				.map((e) => e.system_id),
		);
		...
		const fuelPerJump = route.fuel_per_jump;
		const fuelNeeded =
			route.estimated_fuel + fuelPerJump * target.hopsToHome + this.options.minFuelReserve;
		...
		if ((ship?.fuel ?? 0) < fuelNeeded || hullRatio < this.options.repairThreshold) {
			...
			const freshIntelResult = await ctx.endpoints.queryIntel();
			const freshRecordedIds = new Set(
				(
					freshIntelResult.structuredContent.entries as Array<{
						system_id: string;
						submitted_at_tick?: number;
					}>
				)
			...
			const freshRouteResult = await ctx.endpoints.findRoute(newTarget.systemId);
			const freshRoute = freshRouteResult.structuredContent;
			const freshFuelPerJump = freshRoute.fuel_per_jump;
			const freshFuelNeeded =
				freshRoute.estimated_fuel +
		...
		if (this.options.survey) {
			...
			await ctx.endpoints.surveySystem();
			ticksUsed++;
		}
...
export async function runExplorationLoop(
	...
	let intelLevel = 0;
	try {
		const intelResult = await ctx.endpoints.intelStatus();
		intelLevel = intelResult.structuredContent.intel_level;
	} catch (err) {
	...
	const factory = (_state: StoredGameState): Goal => new ExplorationIteration(iterationOptions);
