...
import { LootUntilFull } from "../compounds/loot-until-full.js";
...
type MapSystem = Extract<
	components["schemas"]["GetMapResponse"],
	{ systems: unknown }
>["systems"][number];
...
class RoamingSalvageIteration implements Goal {
	...
	private async visitNextPoi(
		...
		const homeRouteResult = await ctx.endpoints.findRoute(this.options.homeSystemId);
		const homeRoute = homeRouteResult.structuredContent;
		const fuelToReturn = homeRoute.estimated_fuel + this.options.minFuelReserve;
		...
		if (currentLocation !== currentSystemId) {
			...
			const navResult = await new NavigateToSystem(currentSystemId).execute(ctx);
		...
		// Travel to the POI.
		const poiResult = await new GoToPoi(poiId).execute(ctx);
		...
		const undockResult = await new EnsureUndocked().execute(ctx);
		...
		// Loot all wrecks at this POI.
		const freshState = ctx.refreshState ? await ctx.refreshState() : ctx.state;
		const lootResult = await new LootUntilFull({
	...
	private async advanceToNextSystem(
		...
		const routeResult = await ctx.endpoints.findRoute(target.systemId);
		const route = routeResult.structuredContent;
		const fuelNeeded =
			route.estimated_fuel + route.fuel_per_jump * target.hopsToHome + this.options.minFuelReserve;
		...
		const navResult = await new NavigateToSystem(target.systemId).execute(ctx);
		if (!navResult.success) {
			return navResult;
		}

		// Fetch the system's POI list. Skip POIs with a base (station) — no wrecks spawn there.
		const systemResult = await ctx.endpoints.getSystem(target.systemId);
		...
		const salvagePois = allPois.filter((poi: { has_base: boolean }) => !poi.has_base);
	...
	private async returnHome(ctx: GoalContext, initialTicks: number): Promise<GoalResult> {
		log.info(`Returning home to ${this.options.homeSystemId} to deposit/refuel`);
		let ticksUsed = initialTicks;

		const prepResult = await new PrepareAtStation({
		...
		const dockedState = ctx.refreshState ? await ctx.refreshState() : ctx.state;
		const dockedCtx: GoalContext = {
			endpoints: ctx.endpoints,
			state: dockedState,
			...(ctx.refreshState ? { refreshState: ctx.refreshState } : {}),
		};

		const emptyResult = await new EnsureEmptyCargo({
		...
		const undockResult = await new EnsureUndocked().execute(dockedCtx);
		ticksUsed += undockResult.ticksUsed;
		if (!undockResult.success) {
			return {
				success: false,
...
export async function runRoamingSalvageLoop(
	...
	try {
		const mapResult = await ctx.endpoints.getMap();
		const mapContent = mapResult.structuredContent;
		if (!("systems" in mapContent)) {
			return {
				success: false,
	...
	const factory = (_state: StoredGameState): Goal => new RoamingSalvageIteration(iterationOptions);
