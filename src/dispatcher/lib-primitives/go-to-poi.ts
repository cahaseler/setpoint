...
export class LibGoToPoi implements LibGoal {
	...
	async execute(ctx: LibGoalContext): Promise<GoalResult> {
		...
		if (currentPoiId === this.targetPoiId && !ctx.state.location?.in_transit) {
			// Trustworthy since @spacemolt/lib 14.0.1: the cache learns that a
			// transit has started from the server's own push, clearing the POI just
			// left and setting in_transit. Before that it kept naming the origin for
			// the whole journey, so this check could see its own target, skip the
			// travel, and leave the ship in the right system at the wrong POI.
