...
/**
 * Turn an OwnedPlayerFilter into the predicate `connectOwned` expects.
...
export function buildOwnedFilter(filter?: OwnedPlayerFilter): (player: ClerkPlayer) => boolean {
	...
	return (player: ClerkPlayer): boolean => {
		if (player.hidden && !includeHidden) {
			return false;
		}
		if (usernames && !usernames.includes(player.username.toLowerCase())) {
			return false;
		}
		if (empires && !empires.includes(player.empire.toLowerCase())) {
			return false;
		}
		return true;
	};
